...
class DiceCheck(object):
    def __init__(self):
        self.sides = self.getIntFromUser('Please enter the number of sides for the die to be tested.')
    ...
    def evaluateBalance(self, results):
        ...
        for i in range(self.sides):
            print "D%s, face %s:  %s, %s%%" % (self.sides, i+1, results[i+1], (float(results[i+1])/float(iterations))*100)
    
    def getRollFromUser(self, count, iterations):
        while True:
            tmp = self.getIntFromUser('Please roll the d%s and enter the value. (%s/%s)' % (self.sides, count+1, iterations))
